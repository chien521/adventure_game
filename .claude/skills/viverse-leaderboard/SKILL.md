...
description: How to implement a global leaderboard for VIVERSE worlds using the official VIVERSE gameDashboard SDK. Use when adding, reviewing, or debugging score submission, ranking fetch, or Studio leaderboard setup in this project.
...
## This project (What the Snow Remembers)

`src/viverse/ViverseSession.js` implements `submitScore()`/`fetchLeaderboard()` per this skill. Two leaderboards are used: `VITE_VIVERSE_LEADERBOARD_ANY_COMPLETION` (any full 4-chapter run) and `VITE_VIVERSE_LEADERBOARD_ALL_KEYS` (only if all 4 memory keys collected), both scored by elapsed seconds ascending. Both must exist in VIVERSE Studio under the app's App ID before testing — see [[undertow-viverse-auth-leaderboard]] project memory.
