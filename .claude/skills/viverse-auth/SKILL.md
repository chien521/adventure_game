---
name: viverse-auth
description: VIVERSE Login SDK integration for user authentication and SSO. Use when adding, reviewing, or debugging VIVERSE account login, checkAuth/session handling, profile fetching, or logout in this project.
...
`src/viverse/ViverseSession.js` is this project's implementation of this skill — a vanilla-JS (not React) adaptation. Reuse it instead of writing a second auth path; see [[undertow-viverse-auth-leaderboard]] project memory for why login is opt-in only (not run on every page load).
