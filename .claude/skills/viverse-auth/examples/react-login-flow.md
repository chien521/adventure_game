...
> This project (What the Snow Remembers) is vanilla JS/Three.js, not React — see `src/viverse/ViverseSession.js` for the adapted, non-React version of this same flow. This file is kept as upstream reference material for the auth service-layer shape and mandatory compliance details (token key shotgun, profile fallback chain), which still apply outside React.
